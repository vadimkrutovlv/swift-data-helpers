...
if [ "$#" -lt 2 ] || [ "$#" -gt 3 ]; then
  echo "Usage: $0 <docs_dir> <default_channel> [manifest_name]" >&2
  exit 1
fi

docs_dir="$1"
default_channel="$2"
manifest_name="${3:-versions.json}"
...
escaped_default_channel="${default_channel//\\/\\\\}"
escaped_default_channel="${escaped_default_channel//\"/\\\"}"
escaped_manifest_name="${manifest_name//\\/\\\\}"
escaped_manifest_name="${escaped_manifest_name//\"/\\\"}"

cat > "${docs_dir}/version-switcher.js" <<'EOF'
(() => {
...
DEFAULT_CHANNEL_ESCAPED="${escaped_default_channel}" \
  perl -0pi -e 's/__DEFAULT_CHANNEL__/$ENV{DEFAULT_CHANNEL_ESCAPED}/g' "${docs_dir}/version-switcher.js"

MANIFEST_NAME_ESCAPED="${escaped_manifest_name}" \
  perl -0pi -e 's/__MANIFEST_NAME__/$ENV{MANIFEST_NAME_ESCAPED}/g' "${docs_dir}/version-switcher.js"

loader_snippet="$(cat <<'EOF'
<script id="codex-docs-switcher-loader">(()=>{const seg=location.pathname.split("/").filter(Boolean);const isChannel=s=>s==="main"||s==="stable"||/^\d+\.\d+\.\d+$/.test(s);const i=seg.findIndex(isChannel);if(i===-1)return;const prefix=i===0?"/":`/${seg.slice(0,i).join("/")}/`;const channel=seg[i];const script=document.createElement("script");script.src=`${prefix}${channel}/version-switcher.js`;script.defer=true;document.head.appendChild(script);})();</script>
EOF
)"

while IFS= read -r -d '' file; do
  if grep -q "codex-docs-switcher-loader" "${file}"; then
    continue
  fi
